...
include_once "WSPengurai.inc.php";
...
define("MODE_DICTIONARY_ALL", 1);
define("MODE_DICTIONARY_HANYA_FIELD", 2);

define("FILTER_NONE_STRING", "");
define("FILTER_NONE_INT", -1);
...
{
    /**
     * daftar act yang diambil dari URL WebService (kelas WSPengurai)
     */
    var $WSPengurai;
    /**
     * setting PDDIKTI 
...
/*vv koneksi WEBSERVICE PDDIKTI vv*/
     ...
     * - mode debug?
     */
    function __construct($pddikti, $institusi, $debug=false, $islog=false)
    {
        ...
        $this->WSPengurai           = new WSPengurai($this->pddikti["ws"]["url"]);
        if ($islog)
        {
    ...
    function persiapan()
    {
        /* tes koneksi ke webservice PDDIKTI */
        $this->is_terhubung();
        /*list semua WS*/
        session_start();
        if (!array_key_exists("tembolok_listWS", $_SESSION))
        {
            $this->WSPengurai->ListSemuaWs();
            $_SESSION["tembolok_listWS"]    = $this->WSPengurai->ListWS;
        }
        else
        {
            $this->WSPengurai->ListWS               = $_SESSION["tembolok_listWS"];
        }
        session_write_close();
    ...
    function penokenan()
    {
        if ($this->status_periksa())
        {
            ...
            if (ifkeyexists("expire", $_SESSION, "") == "" || ifkeyexists("expire", $_SESSION, $this->now) < $this->now || $_SESSION["token"] == "")
            {
                /* token digenerate ulang jika sudah melewati masa expire */
                if (ifkeyexists("expire", $_SESSION, "1970-01-01 00:00:00") < $this->now || $_SESSION["token"] == "")
                {
    ...
    {
        /* token */
        $result = $this->runWS(METHOD_GET, "Token", ["username" => $this->pddikti["login"]["username"], "password" => $this->pddikti["login"]["password"]], KELUARGA_READ);
        return ($result["error_code"] == 0) ? $result["data"]["token"] : $result["error_description"];
    }
...
//        }
    ...
    //------------------------- ok v2
    /**
       eksekusi web service v2.2
     * @param type $method 
     * - METHOD_GET/METHOD_UPDATE/METHOD_INSERT/METHOD_DELETE
     * @param type $tabel 
     * - aksi atau tabel, misalnya prodiall, token, dll
     * @param type $data 
     * - berupa array, contoh json: {"act":"GetToken","username":"agus","password":"abcdef"} 
     * @param type $keluarga
     * - keluarga token yang boleh menggunakan perintah ini
     * @param type $type 
     * - json/xml, tapi kita pilih json saja
     * @return type
     */
    function runWS($method, $tabel, $data, $keluarga, $type='json') 
    {
        $act    = $this->WSPengurai->act($method, $tabel, $keluarga);
        return $this->runWSAct($act, $data, $type);
    }
    
    //------------------------- ok v2
    /**
       eksekusi web service v2.2
     * @param type $act  act web service, harus sudah menggunakan act($method, $tabel)
     * @param type $data berupa array, contoh json: {"act":"GetToken","username":"agus","password":"abcdef"} 
     * @param type $type json/xml, tapi kita pilih json saja
     * @return type
     */
    function runWSAct($act, $data, $type='json') 
    {
        if ($act != "ActError") 
        {
            if (empty($data))
            ...
            curl_setopt($ch, CURLOPT_URL, $this->WSPengurai->getURL());
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1); 
            curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
            /*dapatkan hasil, langsung dikonversi ke dalam array*/
            $result = json_decode(curl_exec($ch), true); 
            /*jika gagal, maka ambil pesan error*/
            ...
            {
                /*hitung jumlah data, request dan response*/
                if (array_key_exists("data", $result))
                {
                    if (!array_key_exists("data_count", $result))
                    {
                        $result["data_count"]    = count($result["data"]);
                    }
                    if (array_key_exists("request", $result["data"]))
                    {
                        if (!array_key_exists("request_count", $result["data"]))
                        {
                            $result["data"]["request_count"]    = count($result["data"]["request"]);
                        }
                    }
                    if (array_key_exists("response", $result["data"]))
                    {
                        if (!array_key_exists("response_count", $result["data"]))
                        {
                            $result["data"]["response_count"]   = count($result["data"]["response"]);
                        }
                    }
                }
            }
...
/*vv WEBSERVICE PDDIKTI vv*/
    ...
    {
        /* mendapatkan daftar method get (equal: show [table]) */
        ...
        $header[0] = ["table" => "", "Deskripsi" => "", "ref" => "", METHOD_GET => "", METHOD_GETLIST => "", METHOD_GETDETAIL => "", METHOD_GETREKAP => "", METHOD_EXPORTDATA => "", METHOD_INSERT => "", METHOD_UPDATE => "", METHOD_DELETE => "", "get_prio" => ""];
        $this->cetak_tabel_parsial_indeks($header, 1, false, true);
        /* menyamakan kolom data tabel */
        $daftarTabel  = $this->array_auto_fill($daftarTabel, $header[0]);
        /* mencetak daftar tabel */
        foreach ($daftarTabel as $idx => $data)
        {
        ...
        foreach ($daftarTabel as $idx => $data)
        {
            ...
            /* mencetak header, diambil dari indeks */
            $header[0] = ["Tipe" => "", "Variabel" => "", "Type" => "", "Primary" => "", "Nullable" => "", "Keterangan" => ""];
            /* memulai membuat tabel secara terpisah (harus diakhiri dengan cetak_tabel_parsial_akhiri) */
            $this->cetak_tabel_parsial_mulai();
            /* mencetak header, diambil dari indeks */
            $this->cetak_tabel_parsial_indeks($header, 1, false, true);
            /* menyamakan kolom data tabel */
            $deskripsiTabel["result"]  = $this->array_auto_fill($deskripsiTabel, $header[0]);            
            /* mencetak deskripsi tabel */
    ...
    function ListTable()
    {
        reset($this->WSPengurai->ListWS["ALL"]);
        /* daftar tabel */
        return $this->WSPengurai->ListWS["ALL"];
    }
    ...
    function GetDictionaries()
    {
        ...
        {
            $_SESSION["dictionaries"][MODE_DICTIONARY_ALL]  = array();
            session_write_close();
            foreach ($this->ListTable() as $idx => $data)
            {
                foreach($data["token"] as $datat)
                {
                    if (array_key_exists("table", $data))
                    {
                        $this->GetDictionary($data["table"], MODE_DICTIONARY_ALL, $datat, false);
                    }
                }
            }
            @session_start();
            $_SESSION["GetDictionaries"]    = 1;
        }
        session_write_close();
    }
    ...
    //------------------------- ok v2
    /**
     * mendapatkan deskripsi tabel dari webservice PDDIKTI
     * @param type $tabel
     * - tabel yang akan dilihat deskripsinya
     * @param type $mode
     * - apakah hanya response (MODE_DICTIONARY_HANYA_FIELD),  atau response dan request (MODE_DICTIONARY_ALL)
     * @param type $method
     * - metode yang akan dibuat deskripsinya
     * @param type $getonly
     * - true: hanya KELUARGA_GET, false: semua CRUD
     * @return type
     * - deskripsi tabel
     */
    function GetDictionary($tabel, $mode=MODE_DICTIONARY_ALL, $method=METHOD_GET_OTO, $getonly = true)
    {
        @session_start();
        if (!@array_key_exists($tabel, $_SESSION["dictionaries"][$mode][$method]))
        {
            session_write_close();
            $gd  = $this->WSPengurai->act(METHOD_GET, "Dictionary", KELUARGA_READ);
            $act = ($getonly) ? $this->WSPengurai->actGet($tabel, $method) : $this->WSPengurai->act($method, $tabel, KELUARGA_OTO);
            /* jika koneksi FEEDER DIKTI tidak bermasalah */
            if ($this->status_periksa() && $gd != "" && $act != "ActError")
            {
                /* menjamin keberadaan token, meminta token kembali apabila sudah expire */
                $this->penokenan();
                /* deskripsi tabel */
                $result = $this->runWSAct($gd, ["fungsi" => $act]);
                $ret    = array();
                if (is_array($result["data"]))
                {
                    if ($mode != MODE_DICTIONARY_HANYA_FIELD && array_key_exists("request", $result["data"]))
                    {
                        foreach ($result["data"]["request"] as $idx=>$data)
                        {
                            if (!($idx == "error_code" || $idx == "error_desc"))
                            {
                                $id         = $this->ProperVariabel($idx);
                                $ret[$id]   = ["Tipe" => "Request", "Variabel" => $id, "type" => ifkeyexists("type", $data, ""), "primary" => ifkeyexists("primary", $data, ""), "nullable" => ifkeyexists("nullable", $data, ""), "keterangan" => ifkeyexists("keterangan", $data, "")];
                            }
                        }
                    }
                    if (($mode == MODE_DICTIONARY_HANYA_FIELD || $mode == MODE_DICTIONARY_ALL) && array_key_exists("response", $result["data"]))
                    {
                        foreach ($result["data"]["response"] as $idx=>$data)
                        {
                            if (!($idx == "error_code" || $idx == "error_desc"))
                            {
                                $id         = $this->ProperVariabel($idx);
                                $ret[$id]   = ["id" => $id, "Tipe" => "Response", "Variabel" => $id, "type" => ifkeyexists("type", $data, ""), "primary" => ifkeyexists("primary", $data, ""), "nullable" => ifkeyexists("nullable", $data, ""), "keterangan" => ifkeyexists("keterangan", $data, "")];
                            }
                        }
                    }
                    @session_start();
                    $_SESSION["dictionaries"][$mode][$method][$tabel]  = $ret;
                    session_write_close();
                    return ($result["error_code"] == 0) ? $ret : $result["error_description"];
                }
            }
        }
        else
        {
            $ret = $_SESSION["dictionaries"][$mode][$tabel][$method];
            session_write_close();
            return $ret;
        }
    }
    
    function ProperVariabel($var)
    {
        return str_replace("]", "", str_replace("key[", "", str_replace("record[", "", str_replace("data[", "", $var))));
    }
     ...
     * - satu baris data
     */
    function GetRecord($tabel, $filter="", $order="", $method=METHOD_GET_OTO)
    {
        /* satu baris data */
        return $this->GetRecordset($tabel, $filter, $order, 1, 0, $method);
    }
     ...
     * - data
     */
    function GetRecordset($tabel, $filter="", $order="", $batas=1000000, $mulai=0, $method=METHOD_GET_OTO)
    {
        $act = $this->WSPengurai->actGet($tabel, $method);
        /* jika koneksi FEEDER DIKTI tidak bermasalah */
        if ($this->status_periksa() && $act != "ActError")
        {
            /* menjamin keberadaan token, meminta token kembali apabila sudah expire */
            $this->penokenan();
            /* data */
            $data = array();
            if ($filter != FILTER_NONE_STRING)
            {
                $data["filter"] = $filter;
            }
            if ($order != FILTER_NONE_STRING)
            {
                $data["order"] = $order;
            }
            if ($batas != FILTER_NONE_INT)
            {
                $data["limit"] = $batas;
            }
            if ($mulai != FILTER_NONE_INT)
            {
                $data["offset"] = $mulai;
            }
            $result = $this->runWSAct($act, $data);
            return $result;
        }
    }
    
    function GetRecordsetArray($tabel, $data, $method=METHOD_GET_OTO)
    {
        $act = $this->WSPengurai->actGet($tabel, $method);
        /* jika koneksi FEEDER DIKTI tidak bermasalah */
        if ($this->status_periksa() && $act != "ActError")
     ...
     * - jumlah baris data
     */
    function GetCountRecordset($tabel, $filter="", $order="", $batas=1000000, $mulai=0, $method=METHOD_GET_OTO)
    {
        /* jika koneksi FEEDER DIKTI tidak bermasalah */
        if ($this->status_periksa())
        {
            ...
            $this->penokenan();
            /*dapatkan data*/
            $result = $this->GetRecordset($tabel, $filter, $order, $batas, $mulai, $method);
            return (array_key_exists("data_count", $result)) ? $result["data_count"] : count($result["data"]);
        }
    }
    ...
    {
        $act = $this->WSPengurai->act(METHOD_INSERT, $tabel, KELUARGA_CREATE);
        /* jika koneksi FEEDER DIKTI tidak bermasalah */
        if ($this->status_periksa() && $act != "ActError")
        {
            /* menjamin keberadaan token, meminta token kembali apabila sudah expire */
            $this->penokenan();
            /* data */
            $result = $this->runWSAct($act, ["record" => $data]);
            return $result;
        }
    }
     ...
     * - tabel (PDDIKTI) di mana data disimpan
     * @param type $data
     * - data berupa array 2 dimensi yang indeksnya adalah nama kolom dan isinya adalah data. $sisip = array ( "primarykey1" => array( "namakolom1" => "data1.1", ... ), ... )
     * @return type
     * - status penyisipan data dalam bentuk array ( "primarykey1" => "status1", ... )
     */
    function InsertRecordset($tabel, $data)
    {
        ...
        if ($this->status_periksa() && $act != "ActError")
        {
            /* menjamin keberadaan token, meminta token kembali apabila sudah expire */
            $this->penokenan();
            foreach ($data as $primarykey => $insertdata)
            {
                /* data */
                $result = $this->runWSAct($act, ["record" => $insertdata]);
                $ret[$primarykey] = $result;
            }
            return $ret;
        }
    }
     ...
     *   $ubah = array( "key" => id_pddikti1, "data" => array( "namakolomdata1" => "data1", ... ) )
     * @return type
     * - status ubah data
     */
    function UpdateRecord($tabel, $data)
    {
        $act = $this->WSPengurai->act(METHOD_UPDATE, $tabel, KELUARGA_UPDATE);
        /* jika koneksi FEEDER DIKTI tidak bermasalah */
        if ($this->status_periksa() && $act != "ActError")
        {
            /* menjamin keberadaan token, meminta token kembali apabila sudah expire */
            $this->penokenan();
            /* data */
            $result = $this->runWSAct($act, ["key" => $data["key"], "record" => $data["record"]]);
            return $result;
        }
    }
     ...
     *   $ubah = array ( array( "key" => id_pddikti1, "data" => array( "namakolomdata1.1" => "data1.1", ... ) ) , ... )
     * @return type
     * - status ubah data
     */
    function UpdateRecordset($tabel, $data)
    {
        ...
        if ($this->status_periksa() && $act != "ActError")
        {
            /* menjamin keberadaan token, meminta token kembali apabila sudah expire */
            $this->penokenan();
            foreach ($data as $primarykey => $updatedata)
            {
                /* data */
                $result = $this->runWSAct($act, ["key" => $updatedata["key"], "record" => $updatedata["record"]]);
                $ret[$primarykey] = $result;
            }
            return $ret;
        }
     ...
     * - tabel (PDDIKTI) di mana data disimpan
     * @param type $filter
     * - filter berupa array yang indeksnya bernama 'key'
     *   $hapus = array( "key" => id_pddikti )
     * @return type
     * - status hapus data
     */
    function DeleteRecord($tabel, $filter)
    {
        $act = $this->WSPengurai->act(METHOD_DELETE, $tabel, KELUARGA_DELETE);
        /* jika koneksi FEEDER DIKTI tidak bermasalah */
        if ($this->status_periksa() && $act != "ActError")
        {
            /* menjamin keberadaan token, meminta token kembali apabila sudah expire */
            $this->penokenan();
            /* data */
            $result = $this->runWSAct($act, ["key" => $filter]);
            return $result;
        }
    }
     ...
     * - tabel (PDDIKTI) di mana data disimpan
     * @param type $filter
     * - filter berupa kumpulan array yang indeksnya bernama 'key'
     *   $hapus = array ( array( "key" => id_pddikti1, ... ), ... )
     * @return type
     * - status hapus data
     */
    function DeleteRecordset($tabel, $filter)
    {
        ...
        if ($this->status_periksa() && $act != "ActError")
        {
            /* menjamin keberadaan token, meminta token kembali apabila sudah expire */
            $this->penokenan();
            foreach ($filter as $deletefilter)
            {
                /* data */
                $result = $this->runWSAct($act, ["key" => $deletefilter]);
                $ret[$primarykey] = $result;
            }
            return $ret;
        }
...
/*vv sync DB - WEBSERVICE PDDIKTI vv*/
    ...
    function cek_tabel($perkecualian=array(), $nonref=array())
    {
        ...
        foreach ($listtable as $tabel)
        {
            ...
            if (($tabel["ref"] || in_array($tabel["table"], $nonref)) && !in_array($tabel["table"], $perkecualian))
            { 
                ...
                else
                {
                    ...
                    $listfield  = $this->GetDictionary($tabel["table"], MODE_DICTIONARY_HANYA_FIELD);
                    foreach ($listfield as $idx_field => $kolom)
                    {
                        /* cek mapping kolom PDDIKTI dengan kolom Institusi */
                        echo "<br />".((ifkeyexists($idx_field, $this->mapdb["field"][$tabel["table"]], "")=="") ? "Warning! Field ".$tabel["table"].".".$idx_field." belum dipetakan" : "Field ".$tabel["table"].".".$idx_field." dipetakan ke field ".$this->mapdb["table"][$tabel["table"]]["nama"].".".$this->mapdb["field"][$tabel["table"]][$idx_field]);
                    }
...
//                else 
                {
                    if ($pemetaanpk && !(count($this->mapdb["table"][$tabel["table"]]["nama"])==0 || $this->mapdb["table"][$tabel["table"]]["nama"]==""))
                    {
                         ...
                         */
                        while($row = $this->db["result"][$iddb]->fetch_array())
                        {
                            ...
                            for ($i=0; $i<$kolom["count"]; $i++)
                            {
                                ...
                                {
                                    if (!array_key_exists($i, $tabel_ref["data"][$indeks]))
                                    {
                                        $tabel_ref["data"][$indeks][$i] = "";
                                    }
                    ...
                    /* header */
                    foreach ($hasil["data"][0] as $idx => $value)
                    {
...
//                    if ($pemetaanpk)
                    {
                        ...
                        $brs    = $this->GetCountRecordset($tabel["table"]);
                                ...
                                else
                                {
                                    ...
                                    foreach ($rec["result"] as $idx => $val)
                                    {
                                        ...
                                        if ($val[$mapdb_guid["forcedouble"]["field"]] != $upd)
                                        {
                                            ...
                                            {
                                                $row["ket"] .= "<br />Error: ".ifnull($hasil["result"]["error_desc"], $hasil["error_desc"])." UpdateRecordset(".$mapdb_guid["forcedouble"]["table"].", ".$this->cetak_array($dataupdate, true).")";
                                            }
                ...
                if ($inject["type"]=="insert")
                {
                  ...
                  while($row = $this->db["result"][$iddb]->fetch_assoc())
                  {
                    ...
                    if ($hasil["result"]["error_desc"] != "" || $hasil["error_desc"] != "")
                    {
                      $error++;
                      $row["no sync"] = $proses;
                      $row["ket"]     = ifnull($hasil["result"]["error_desc"], $hasil["error_desc"])." InsertRecord(".$tabel_asli.", ".$this->cetak_array($data, true).")";
                      $this->cetak_tabel_parsial($row);
                    }
                    else
                    {
...
//                        $upd  = "concat(".implode(",", $upd).")";
                ...
                else if ($inject["type"]=="update")
                {
                  ...
                  while($row = $this->db["result"][$iddb]->fetch_assoc())
                  {
                    ...
                    if ($hasil["result"]["error_desc"] != "" || $hasil["error_desc"] != "")
                    {
                      $error++;
                      $row["no sync"] = $proses;
                      $row["ket"]     = ifnull($hasil["result"]["error_desc"], $hasil["error_desc"])." UpdateRecord(".$tabel_asli.", ".$this->cetak_array($data, true).")";
                      $this->cetak_tabel_parsial($row);
                    }
                    else
                    {
     ...
     */
    function array_auto_fill($data, $model = "")
    {
      ...
      foreach ($data as $idx => $val)
      {
        ...
        foreach ($idxList as $idx_ => $val_)
        {
            ...
            $tempData[$idx][$idx_]  = (array_key_exists($idx_, $val) ? $val[$idx_] : "");
        }
      }
      /* bersih-bersih */
      unset($idxList);
...
//        echo "</pre>";
    ...
    function get_url()
    {
        return (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://".$_SERVER["HTTP_HOST"].$_SERVER["REQUEST_URI"]." -- ".ifkeyexists("HTTP_REFERER", $_SERVER, "");
    }
